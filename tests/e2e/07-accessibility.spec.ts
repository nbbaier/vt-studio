...
test.describe('Accessibility', () => {
  ...
  test('should be keyboard navigable - token config', async ({ page }) => {
    ...
    // Should connect
    const hasStudio = await studioPage.studioContainer.isVisible().catch(() => false);
  ...
  test('should have semantic HTML structure', async ({ page }) => {
    ...
    const executeButton = await page.locator('button[data-testid="execute-query-btn"]');
  ...
  test('should support screen reader announcements for errors', async ({ page }) => {
    await helpers.setValtownToken(TEST_DATA.validToken);
