...
test.describe('Error Handling', () => {
  ...
  test('should handle API timeout', async ({ page }) => {
    ...
    await page.route('https://api.val.town/v1/sqlite/execute', async (route) => {
  ...
  test('should clear error when executing successful query', async ({ page }) => {
    await helpers.setValtownToken(TEST_DATA.validToken);
  ...
  test('should show user-friendly error messages', async ({ page }) => {
    ...
    await studioPage.typeSQL("INSERT INTO users (email) VALUES ('test@example.com')");
    await studioPage.executeQuery();
