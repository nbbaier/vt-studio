...
export class ValtownAPIMock {
   ...
   */
  async mockSuccessfulQuery(result: {
    columns: string[];
    rows: any[][];
  }) {
    await this.page.route('https://api.val.town/v1/sqlite/execute', async (route) => {
      await route.fulfill({
        status: 200,
  ...
  async mockBatchQuery(results: Array<{ columns: string[]; rows: any[][] }>) {
    await this.page.route('https://api.val.town/v1/sqlite/batch', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  async mockUnauthorized() {
    await this.page.route('https://api.val.town/v1/sqlite/**', async (route) => {
      await route.fulfill({
        status: 401,
        contentType: 'application/json',
        body: JSON.stringify({
