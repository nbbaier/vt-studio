...
test.describe('Studio Interface', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('/');
    await helpers.setValtownToken(TEST_DATA.validToken, TEST_DATA.connectionName);
  });
  ...
  test('should show schema sidebar', async ({ page }) => {
    ...
    const hasSchemaTree = await studioPage.schemaTree.isVisible().catch(() => false);
  ...
  test('should support multiple tabs', async ({ page }) => {
    ...
    const hasQueryTab = await studioPage.queryTab.isVisible().catch(() => false);
    expect(hasQueryTab).toBe(true);

    // New tab button should exist
    const hasNewTabButton = await studioPage.newTabButton.isVisible().catch(() => false);
    if (hasNewTabButton) {
