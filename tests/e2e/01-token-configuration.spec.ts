...
test.describe('Token Configuration Flow', () => {
  ...
  test('should show token configuration UI when no token exists', async ({ page }) => {
  ...
  test('should persist connection name', async ({ page }) => {
    ...
    // Connect with name
    await tokenConfigPage.connectWithToken(TEST_DATA.validToken, TEST_DATA.connectionName);
