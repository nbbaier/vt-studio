...
test.describe('Schema Browsing', () => {
  ...
  test('should open table when clicked', async ({ page }) => {
    ...
    // Click on a table
    const hasTableList = await studioPage.tableList.isVisible().catch(() => false);
    if (hasTableList) {
  ...
  test('should show column types in schema', async ({ page }) => {
    ...
    const hasTypes = pageContent.includes('INTEGER') || pageContent.includes('TEXT');
  ...
  test('should show views in schema sidebar', async ({ page }) => {
    ...
    const hasViews = await viewItem.isVisible().catch(() => false);
